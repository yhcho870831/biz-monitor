...
def format_summary_lines(summary_payload: dict, *, prefix: str = "") -> list[str]:
    ...
    def _split_summary_value(value: str) -> list[str]:
        parts = []
    ...
    def _clip_part(value: str) -> str:
        return value if len(value) <= 180 else value[:179] + "…"

    lines: list[str] = []
    if title_prefix:
        lines.append(f"{title_prefix}요약")
    for label, key in (
        ("사업목적", "purpose"),
        ("핵심수행업무", "core_tasks"),
        ("요구성능", "required_performance"),
        ("정량 목표", "quantitative_targets"),
        ("기간", "period_text"),
    ):
        lines.append(f"{label}")
        for part in _split_summary_value(summary_payload[key]):
            lines.append(f"  - {_clip_part(part)}")
    return lines
