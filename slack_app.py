...
def _format_keyword_list(rows: list[tuple[str, str]]) -> str:
    ...
    lines = ["\uac80\uc0c9\uc5b4\ub97c \uc870\ud68c\ud588\uc2b5\ub2c8\ub2e4.", ""]
    for site_code in sorted(grouped.keys()):
        lines.append("[%s]" % SITE_DISPLAY_NAMES.get(site_code, site_code.upper()))
        for keyword in grouped[site_code]:
...
def build_slack_app(settings: Settings, session_factory, collector_registry, notifier):
    ...
    def handle_app_mention(body, logger):  # pragma: no cover - external event loop
        ...
        if command.action not in {"list_keywords", "list_notices", "calendar_link"} and command.site_code not in collector_registry:
            reply(
                channel_id,
                event_ts,
