...
class SummaryServiceTest(unittest.TestCase):
    ...
    def test_quantitative_targets_excludes_background_lines(self) -> None:
        notice_id = self._create_notice()
        attachment_path = self.output_dir / "quant-target-filter.hwpx"
        self._create_hwpx(
            attachment_path,
            "\n".join(
                [
                    "제안요청서",
                    "사업목적",
                    "AI 모델 개발을 통해 분석 효율을 높인다.",
                    "추진배경 및 필요성",
                    "440개 영상, 80시간 분량의 국내 최다 독도 수중영상 데이터 축적",
                    "주요 선진국 연구기관에서는 다양한 분야에서 활용 중",
                    "성과목표",
                    "어류 15종 식별률 90% 이상",
                    "과업기간: 계약일로부터 2026년 11월 30일까지",
                ]
        ...
        with self.session_factory() as session:
            upsert_notice_attachment(
                ...
                notice_id=notice_id,
                site_code="g2b",
                attachment_name="quant-target-filter.hwpx",
                attachment_category="proposal_request",
                priority_rank=1,
                stored_path=str(attachment_path),
                source_url="https://example.com/file",
            ...
            self.assertEqual(result["quantitative_targets"], "어류 15종 식별률 90% 이상")

    def test_quantitative_targets_falls_back_to_unknown(self) -> None:
        notice_id = self._create_notice()
        attachment_path = self.output_dir / "quant-target-missing.hwpx"
        self._create_hwpx(
            attachment_path,
            "\n".join(
                [
        ...
        with self.session_factory() as session:
            upsert_notice_attachment(
                ...
                notice_id=notice_id,
                site_code="g2b",
                attachment_name="quant-target-missing.hwpx",
                attachment_category="proposal_request",
                priority_rank=1,
                stored_path=str(attachment_path),
                source_url="https://example.com/file",
            ...
            self.assertEqual(result["quantitative_targets"], "미확인")
